...
stat.begin()
dynamic_axes = [
    DynamicAxisInfo("point_coords", min_value=(32, 2, 2), opt_value=(32, 2, 2), max_value=(64, 50, 2)),
    DynamicAxisInfo("point_labels", min_value=(32, 2), opt_value=(32, 2), max_value=(64, 50)),
    DynamicAxisInfo("orig_im_size", min_value=(1000, 1000), opt_value=(2000, 2000),
                    max_value=(6000, 6000))
]
config = Config("vit_h_decoder.onnx", "vit_h_decoder.trt", backend=Backend.TensorRT, dynamic_axes=dynamic_axes,
                devices=[Device.GPU, Device.CPU], enable_log=True)
runtime = Runtime(config)

# 用 numpy 替代 torch 的张量
dummy_inputs = {
    "image_embeddings": np.random.randn(1, 256, 64, 64).astype(np.float32),
    "point_coords": np.random.randint(low=0, high=1024, size=(32, 2, 2)).astype(np.float32),
    "point_labels": np.random.randint(low=0, high=4, size=(32, 2)).astype(np.float32),
